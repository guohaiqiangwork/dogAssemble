  ...
  <div id="eat-book">
    ...
    <div class="mt-2">
      <div class="opt-list flex-between align-center">
        <div>
          <img class="kind-icon" src="../../assets/images/list.png" alt>
          <span class="book-kind">全部订单</span>
        </div>
      </div>
        ...
        <ul class="mt-15" v-for="(item,index) in orderList" :key="index">
          <li class="book-item flex-between align-center" @click="goOrderBook(item.id,item.type)">
            <div>
              ...
              <p>
                <span>{{item.name}}</span>
                <span class="ml-3">{{item.recipe}}</span>
              </p>
            </div>
            <div class="complete-btn" v-if="item.isComplete == 0" @click.stop="finish(item)">完成</div>
          </li>
...
export default {
  ...
  methods: {
    ...
    getOrder() {
      this.$fetch
        .post("fruits/app/recipe/getTodayOrder", {
          openId: localStorage.getItem("openId")
        })
        .then(res => {
          if (res.msg == "success") {
    ...
    finish(item) {
      ...
      this.$fetch
        .post("fruits/app/recipe/completeOrder", this.form)
        .then(res => {
          if (res.msg == "success") {
            item.isComplete = 1;
