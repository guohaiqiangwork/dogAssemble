...
window.settitle = settitle;
 ...
 router.beforeEach((to, from, next) => {
  // && to.name!="answer" && to.name!="result"&& to.name!="nearby" 答题和结果
   if(!localStorage.getItem('user') && to.name !== "login" && to.name !== "shop" && to.name !=="home" && to.name!="changePassword" && to.name !=="introduce"&& to.name !=="sharedBonusTwo"&& to.name !=="nationalStores" && to.name !=="privacyProtocol"&& to.name !=="sharedBonus"&& to.name !=="symptoms"){
     next("/login/1");
     return 
   }
   next();
